...
## Step 0 — まずシステムライブラリを入れる（必須）

> [!IMPORTANT]
> `pycairo` / `PyGObject` / `Pango` は **C 拡張で、pip では確実に入らない**。
> 下記のどのインストール方法より**先に**、OS のパッケージマネージャから入れること。
> mdview が起動しない原因の大半はこれなので、最初に済ませる。
...
先に進む前に、これらが import できることを**確認**する。次の 1 行が `system libs OK`
と表示されれば OK：
...
エラーが出る場合はまずシステムパッケージを直すこと。`system libs OK` が出るまで、
下記のどのインストール方法も動かない。
...
2 つの方法から選ぶ。**Option A**（`uv`）はグローバルな `mdview` コマンドを入れる。
**Option B**（`install.sh`）はローカルに `.venv` を作り、依存ドクターも兼ねる。

### Option A — `uv`（グローバルコマンド）

[uv](https://docs.astral.sh/uv/) を使う人向け。Git から直接インストールする：
...
> [!NOTE]
> `uv tool` は**隔離環境**にインストールするため、Step 0 で入れたシステムの
> `pycairo`/`PyGObject` が**見えない**。`PYTHONPATH` でディストリのパッケージを
> 指して mdview から見えるようにする：
>
> ```bash
> # システムの cairo/gi があるディレクトリを解決する
...
> ```
>
> 毎回打たずに済ませるには、`~/.bashrc` / `~/.zshrc` にエイリアスを足す：
>
> ```bash
...
> [!WARNING]
> PyPI 上の配布名 `mdview` は**無関係な別プロジェクト**。`pip install mdview` /
> `uv tool install mdview` は**しないこと**（別物が入る）。必ず上記の Git URL から
> インストールする。

### Option B — `install.sh`（ローカル `.venv` + ドクター）

自己完結したチェックアウトが好みの人向け。インストーラが OS を検出し、何が入っていて
何が足りないかを正確な導入コマンド付きで表示し、pip パッケージをローカル `.venv` に
導入する：
...
`~/.local/bin` が `PATH` に通っていれば、グローバルな `mdview` コマンドの symlink 作成も
提案される（実行前に確認される）。

<details>
<summary>手動セットアップ（スクリプトも uv も使わない場合）</summary>

Step 0 のあと、システムの pycairo/PyGObject が見える venv を作って pip パッケージを
入れる：
...
（install.sh でグローバル symlink を作らなかった場合のコマンドは `.venv/bin/mdview`。
手動チェックアウトからは `python -m mdview` も使える。）
...
```
mdview/
├── __init__.py   # パッケージマーカー・__version__ の単一管理元
├── __main__.py   # `python -m mdview` を可能にする
...
pyproject.toml    # パッケージ設定（build-system・エントリポイント）+ ruff/pytest 設定
