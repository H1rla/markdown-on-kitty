...
#!/usr/bin/env bash
# Auto-generated by install.sh — runs mdview via the venv's own interpreter.
# Calling .venv/bin/python directly (instead of 'source activate') avoids
# activate's hard-coded VIRTUAL_ENV path, which goes stale if this folder is
# renamed/moved — the venv python locates its own packages regardless.
# mdview is a package using relative imports, so it must be started with
# 'python -m mdview' (running the .py file directly would break the
# intra-package imports). PYTHONPATH exposes the repo root so -m can find it.
exec env PYTHONPATH="$ROOT\${PYTHONPATH:+:\$PYTHONPATH}" "$VENV/bin/python" -m mdview "\$@"
EOF
